...
class PostController extends Controller {
  ...
  async getAllPosts(req, res) {
    ...
    const posts = await PostModel.find(dbQuery, { comments: 0 })
      .populate([
        ...
        { path: "author", select: { name: 1, biography: 1, avatar: 1 } },
        {
          path: "related",
          model: "Post",
          select: { title: 1, slug: 1, bfireText: 1, coverImage: 1, author: 1 },
          populate: [
    ...
    const totalPages = Math.ceil((await PostModel.find(dbQuery)).length / limit);
  ...
  async getPostBySlug(req, res) {
    ...
    transformedPost.commentsCount =
      acceptedComments.length + acceptedComments.reduce((a, c) => a + c.answers.length, 0);
  ...
  async addNewPost(req, res) {
    ...
    if (!fileUploadPath || !filename)
      throw createHttpError.InternalServerError("Upload a cover image for the post");
    const fileAddress = path.join(fileUploadPath, filename);
    const coverImage = fileAddress.replace(/\\/g, "/");

    const post = await PostModel.create({ title, briefText, slug, type, category, tags, text, readingTime, related, author, coverImage });
    if (!post?._id) throw createHttpError.InternalServerError("Post creation failed");
  ...
  async updatePost(req, res) {
    ...
    const updatePostResult = await PostModel.updateOne({ _id: id }, { $set: { ...data, coverImage } });
    if (!updatePostResult.modifiedCount) throw createHttpError.InternalServerError("Post update failed");
  ...
  async removePost(req, res) {
    ...
    if (!post._id) throw createHttpError.InternalServerError("Post deletion failed");
  ...
  async findPostById(id) {
    if (!mongoose.isValidObjectId(id)) throw createHttpError.BadRequest("Invalid post ID");
    const post = await PostModel.findById(id);
    if (!post) throw createHttpError.BadRequest("Post not found");
  ...
  async likePost(req, res) {
    ...
    if (postUpdate.modifiedCount === 0 || userUpdate.modifiedCount === 0) throw createHttpError.BadRequest("Operation failed");

    const message = !likedPost ? "Thanks for liking this post" : "Your like has been removed";
    return res.status(HttpStatus.OK).json({ statusCode: HttpStatus.OK, data: { message } });
  }
  ...
  async bookmarkPost(req, res) {
    ...
    const updatePostQuery = bookmarkedPost ? { $pull: { bookmarks: user._id } } : { $push: { bookmarks: user._id } };
    const updateUserQuery = bookmarkedPost ? { $pull: { bookmarkedPosts: post._id } } : { $push: { bookmarkedPosts: post._id } };
    ...
    const message = !bookmarkedPost ? "Post bookmarked" : "Post removed from bookmarks";
    return res.status(HttpStatus.OK).json({ statusCode: HttpStatus.OK, data: { message } });
  }
}
