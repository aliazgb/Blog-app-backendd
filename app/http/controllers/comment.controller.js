...
class CommentController extends Controller {
  async addNewComment(req, res) {
    ...
    if (parentId && mongoose.isValidObjectId(parentId)) {
      ...
      if (parentComment && !parentComment?.openToComment)
        throw createHttpError.BadRequest("Replying to this comment is not allowed");
      ...
      if (!createAnswerResult.matchedCount && !createAnswerResult.modifiedCount)
        throw createHttpError.InternalServerError("Reply creation failed");

      return res.status(HttpStatus.CREATED).json({
        statusCode: HttpStatus.CREATED,
        data: {
          message: "Your reply was successfully added and will be visible after approval",
        },
      });
    } else {
      ...
      if (!newComment)
        throw createHttpError.InternalServerError("Comment creation failed");
      return res.status(HttpStatus.CREATED).json({
        statusCode: HttpStatus.CREATED,
        data: {
          message: "Your comment was successfully added and will be visible after approval",
        },
      });
    }
  }
  ...
  async removeComment(req, res) {
    ...
    if (comment && comment.openToComment) {
      ...
      if (!commentToDelete)
        throw new createHttpError.InternalServerError("Comment deletion failed");
      return res.status(HttpStatus.OK).json({
        statusCode: HttpStatus.OK,
        data: {
  ...
  async findAcceptedComments(id, status = 2) {
          ...
          user: {
            $map: {
              ...
              in: {
                $mergeObjects: [
                  "$$item",
                  { avatarUrl: { $concat: [process.env.SERVER_URL, "/", "$$item.avatar"] } },
                ],
              },
            },
          },
    ...
    const transformed = acceptedComments.map((c) => ({
      ...
      answers: c.answers.map((c) => ({ ...c, createdAt: calculateDateDuration(c.createdAt) })),
    }));
